...
/* ============================================================
   蒙蒙英語會話教室 2.0 — 即時語音互動版（6 歲）
   ・🎤 跟讀檢查：孩子對麥克風唸，瀏覽器語音辨識 → 寬鬆比對 → 即時回饋
   ・☕ 跟蒙蒙聊天：說話／點建議句／打字 → Claude API 讓蒙蒙即時回一句
     簡單英文（附中文）→ TTS 唸出，並給「你可以這樣說」建議
   ・沒有麥克風也能玩：全部語音功能都可跳過或改用點選／打字
...
// ---------- 語音辨識（Web Speech API）：寬鬆比對、只鼓勵不扣分 ----------
const SR = () => { try { return window.SpeechRecognition || window.webkitSpeechRecognition || null; } catch (e) { return null; } };
const hasSTT = () => !!SR();
function listenOnce(timeoutMs = 7000) {   // resolve {alts:[...]} 或 {error}
  return new Promise(resolve => {
    const Rec = SR();
    if (!Rec) { resolve({ error: "unsupported" }); return; }
    let done = false;
    ...
    try {
      ...
      rec = new Rec();
      rec.lang = "en-US"; rec.interimResults = false; rec.maxAlternatives = 4; rec.continuous = false;
      rec.onresult = ev => {
        const alts = [];
        for (let i = 0; i < ev.results.length; i++) for (let j = 0; j < ev.results[i].length; j++) alts.push(ev.results[i][j].transcript);
        finish({ alts });
      };
      rec.onerror = ev => finish({ error: ev.error || "error" });
      rec.onend = () => finish({ error: "no-speech" });
      rec.start();
    } catch (e) { finish({ error: "start-failed" }); }
    const tm = setTimeout(() => { try { rec && rec.stop(); } catch (e) { /* noop */ } finish({ error: "timeout" }); }, timeoutMs);
  });
}
const CONTR = { "i'm": "i am", "it's": "it is", "what's": "what is", "let's": "let us", "you're": "you are", "don't": "do not", "can't": "can not", "here's": "here is", "how's": "how is", "that's": "that is", okay: "ok", yeah: "yes", yep: "yes" };
const norm = s => s.toLowerCase().replace(/[^a-z0-9' ]/g, " ").split(/\s+/).filter(Boolean).map(w => CONTR[w] || w).join(" ");
function lev(a, b) {
...
function matchScore(said, target) {
  ...
  if (!A || !B) return 0;
  const a = A.split(" "), b = B.split(" "), pool = b.slice();
  let hit = 0;
  for (const w of a) { const i = pool.indexOf(w); if (i >= 0) { hit++; pool.splice(i, 1); } }
  const wordScore = hit / b.length;
  const charScore = 1 - lev(A, B) / Math.max(1, B.length);
  return { score: Math.max(wordScore, charScore * 0.85), wordScore };
}
function bestMatch(alts, target) {
  let best = { text: alts[0] || "", score: 0, wordScore: 0 };
  for (const t of alts) { const m = matchScore(t, target); if (m.score > best.score) best = { text: t, ...m }; }
  return best;
}
// 「很棒」要同時字詞命中 ≥75%（避免 sunny/funny 這種一字之差被字元相似放水）；「接近」≥50% 就給鼓勵
const tierOf = bm => (bm.score >= 0.8 && bm.wordScore >= 0.75 ? "great" : bm.score >= 0.5 ? "close" : "retry");
const STT_ERR = {
  unsupported: "這個瀏覽器沒有語音辨識，改用點選或打字吧",
  "not-allowed": "麥克風被擋住了（Claude 預覽視窗裡常會這樣；放到自己的網頁就可以）",
  "service-not-allowed": "麥克風被擋住了（Claude 預覽視窗裡常會這樣；放到自己的網頁就可以）",
  "audio-capture": "找不到麥克風",
  "no-speech": "沒聽到聲音，再說一次？", timeout: "沒聽到聲音，再說一次？", network: "語音辨識需要網路", "start-failed": "麥克風啟動失敗",
};
// ---------- 跟蒙蒙聊天：Claude API（回覆一定是簡單英文＋中文＋建議句）----------
async function askMomo(history, childText, learned) {
  const system = `You are Momo (蒙蒙), a kind owl English teacher chatting with a 6-year-old beginner from Taiwan. ` +
    `Reply with JSON only, no markdown: {"en":"ONE short, simple American-English sentence (max 8 words) as Momo's reply, may include a tiny follow-up question","zh":"該句的繁體中文翻譯","hints":["2 or 3 very short things the child could say next in English, max 6 words each, use the child's known lines when possible"]}. ` +
    `Use only very simple words. Known lines the child has learned: ${learned.join(" | ")}. ` +
    `If the child says something in Chinese, reply in simple English about it and put the English way to say it in hints. Be warm and encouraging. Never ask for personal information.`;
  const messages = [...history, { role: "user", content: childText }];
  ...
  return parseMomo(text);
}
function parseMomo(text) {
  const clean = (text || "").replace(/```json|```/g, "").trim();
  try {
    const j = JSON.parse(clean.slice(clean.indexOf("{"), clean.lastIndexOf("}") + 1));
    if (j && j.en) return { en: String(j.en).trim(), zh: String(j.zh || "").trim(), hints: Array.isArray(j.hints) ? j.hints.slice(0, 3).map(String) : [], raw: clean };
  } catch (e) { /* 非 JSON */ }
  const firstLine = clean.split("\n")[0] || "Let's talk more!";
  return { en: firstLine.slice(0, 80), zh: "", hints: [], raw: clean };
}
...
export default function App() {
  ...
  const [mic, setMic] = useState({ status: "idle", result: null, error: null });
  const [micBlocked, setMicBlocked] = useState(false);
  const [voice, setVoice] = useState(0);                 // 🎤 開口成功次數
  const [chat, setChat] = useState([]);                  // [{who, en, zh}]
  const [chatHints, setChatHints] = useState([]);
  const [chatInput, setChatInput] = useState("");
  const [chatBusy, setChatBusy] = useState(false);
  const chatHist = useRef([]);
  const voiceRef = useRef(0); voiceRef.current = voice;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (sv) {
        ...
        setLearner(fresh);
        setTotalStars(sv.stars || 0); setStickers(sv.stickers || []); setSessions(sv.sessions || 0); setSeeds(sv.seeds || 0); setVoice(sv.voice || 0);
      }
    })();
  }, []);
  async function saveAll(st, tStars, stk, ses, sd) {
    ...
    await persist({ it, stars: tStars, stickers: stk, sessions: ses, seeds: sd, voice: voiceRef.current, t: Date.now() });
  }
  ...
  function enterSpeak() {
    setMic({ status: "idle", result: null, error: null });
    setStage("speak");
    setTimeout(() => speak(lessonItem.item.en), 300);
  }
  // ===== 🎤 跟讀檢查：寬鬆比對、只鼓勵不扣分 =====
  async function speakCheck(target) {
    if (micBusy.current) return;
    micBusy.current = true;
    fx("tap");
    setMic({ status: "listening", result: null, error: null });
    const r = await listenOnce(7000);
    micBusy.current = false;
    if (r.error) {
      if (r.error === "not-allowed" || r.error === "service-not-allowed" || r.error === "unsupported") setMicBlocked(true);
      setMic({ status: "idle", result: null, error: STT_ERR[r.error] || "再試一次？" });
      return;
    }
    const bm = bestMatch(r.alts, target);
    const tier = tierOf(bm);
    setMic({ status: "idle", result: { text: bm.text, score: bm.score, tier }, error: null });
    if (tier !== "retry") { fx("good"); const v = voice + 1; setVoice(v); voiceRef.current = v; saveAll(learnerRef.current, totalStars, stickers, sessions, seeds); }
    else fx("soft");
  }
  // ===== ☕ 跟蒙蒙聊天 =====
  const learnedLines = () => ALL.filter(w => learnerRef.current.ws[w.id].seen > 0 || learnerRef.current.ws[w.id].s > 0.02).map(w => w.en).slice(0, 24);
  const defaultHints = () => {
    const l = ALL.filter(w => learnerRef.current.ws[w.id].seen > 0 || learnerRef.current.ws[w.id].s > 0.02).map(w => w.prompt);
    const base = l.length >= 3 ? shuffled(l).slice(0, 3) : ["Hello!", "How are you?", "Let's play!"];
    return base;
  };
  function startChat() {
    chatHist.current = [];
    const opener = { who: "owl", en: "Hello! I'm Momo. How are you today?", zh: "哈囉！我是蒙蒙。你今天好嗎？" };
    setChat([opener]); setChatHints(["I'm fine, thank you!", "I'm happy!", "Hello, Momo!"]); setChatInput("");
    setMic({ status: "idle", result: null, error: null });
    setPhase("chat");
    setTimeout(() => speak(opener.en), 400);
  }
  async function chatSend(text) {
    const t = (text || "").trim();
    if (!t || chatBusy) return;
    fx("tap");
    setChat(c => [...c, { who: "chick", en: t, zh: "" }]);
    setChatInput(""); setChatBusy(true);
    let rep;
    try { rep = await askMomo(chatHist.current, t, learnedLines()); }
    catch (e) { rep = { en: "Sorry, can you say that again?", zh: "抱歉，可以再說一次嗎？（連線不順）", hints: defaultHints(), raw: "" }; }
    chatHist.current = [...chatHist.current, { role: "user", content: t }, { role: "assistant", content: rep.raw || JSON.stringify({ en: rep.en, zh: rep.zh, hints: rep.hints }) }].slice(-12);
    setChat(c => [...c, { who: "owl", en: rep.en, zh: rep.zh }]);
    setChatHints(rep.hints && rep.hints.length ? rep.hints : defaultHints());
    setChatBusy(false);
    speak(rep.en);
  }
  async function chatListen() {
    if (micBusy.current || chatBusy) return;
    micBusy.current = true;
    fx("tap");
    setMic({ status: "listening", result: null, error: null });
        ...
        {phase === "home" && (
          <div>
            <header style={{ textAlign: "center", margin: "8px 0 14px" }}>
              ...
              <h1 style={{ margin: "4px 0 0", fontSize: 26, fontWeight: 900, letterSpacing: 2, color: "#fff", textShadow: `0 3px 0 ${K.skyDeep}` }}>蒙蒙英語會話教室 2.0</h1>
              <p style={{ margin: "4px 0 0", fontSize: 13, color: "#f2fff9", fontWeight: 700 }}>一輪對話・逐字學・照樣造句・🎤 開口練習，還能跟蒙蒙即時聊天 ☕</p>
            </header>
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div style={{ fontWeight: 900, fontSize: 14 }}>🌱 上課 <span style={{ fontFamily: MONO }}>{seeds}</span>　⭐ 星星 <span style={{ fontFamily: MONO }}>{totalStars}</span>　🎤 開口 <span style={{ fontFamily: MONO }}>{voice}</span>　🏅 學會 <span style={{ fontFamily: MONO }}>{mastered.length}</span>/{ALL.length}</div>
            ...
            <Card>
              ...
              <div style={{ marginTop: 10 }}>
                <Btn onClick={() => { fx("tap"); startChat(); }} color={K.pink} edge="#c2588a" fg="#fff" style={{ width: "100%", fontSize: 16, padding: "14px 8px" }}>
                  <span style={{ fontSize: 24 }}>☕</span> 跟蒙蒙聊天
                  <div style={{ fontSize: 11, color: "#fff0f6", fontWeight: 700 }}>即時語音互動：說話、點建議句或打字，蒙蒙都會回你{hasSTT() && !micBlocked ? "・🎤 可用" : "・這裡沒有麥克風，用點選／打字"}</div>
                </Btn>
              </div>
            ...
            <ParentNotes learner={learner} notes={null} sessions={sessions} seeds={seeds} onReset={async () => {
              ...
              setTotalStars(0); setStickers([]); setSessions(0); setSeeds(0); setVoice(0);
              await persist({ it: {}, stars: 0, stickers: [], sessions: 0, seeds: 0, voice: 0, t: Date.now() });
            }} />
          </div>
        )}
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <div style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: "#fff", background: K.sunEdge, borderRadius: 10, padding: "3px 9px" }}>
                📚 第 {lessonIdx + 1} / {TEACH_N} 組・{stage === "dlg" ? "① 聽對話" : stage === "words" ? `② 學單字 ${wordIdx + 1}/${wordList.length}` : stage === "pattern" ? "③ 照樣造句" : "④ 開口練習 🎤"}
              </div>
            </div>
              ...
              {stage === "pattern" && lessonItem.item.pattern && (
                <div>
                  ...
                  {patDone.length >= lessonItem.item.pattern.subs.length ? (
                    <div style={{ marginTop: 12 }}><Btn onClick={() => { fx("tap"); enterSpeak(); }} color={K.teal} edge={K.tealEdge} fg="#fff" disabled={isSpeaking}>{isSpeaking ? "🔊 蒙蒙說話中…" : "✅ 三句都造好了！去開口練習"}</Btn></div>
                  ) : <div style={{ fontSize: 11.5, color: K.sub, marginTop: 10 }}>點一個替換詞，聽新句子（{patDone.length}/{lessonItem.item.pattern.subs.length}）</div>}
                </div>
              )}
              {stage === "speak" && (
                <div>
                  <div style={{ display: "flex", justifyContent: "center" }}><Bubble who="owl" en={lessonItem.item.prompt} zh={lessonItem.item.promptZh} small onClick={() => { fx("tap"); speak(lessonItem.item.prompt); }} tail="🔊" /></div>
                  <MicPanel target={lessonItem.item.en} mic={mic} micBlocked={micBlocked}
                    onListen={() => speakCheck(lessonItem.item.en)}
                    onDone={() => { fx("tap"); finishTeachItem(); }} />
                  <div style={{ fontSize: 11.5, color: K.sub, marginTop: 8 }}>（先聽蒙蒙唸一次 🔊，再按 🎤 自己說。辨識小朋友的聲音本來就比較難，所以蒙蒙很寬鬆、只鼓勵不扣分）</div>
                </div>
              )}
          ...
          <div>
            ...
            {phase === "reveal" && (
              <div style={{ textAlign: "center", marginTop: 12 }}>
                <MicPanel compact target={q.mode === "build" ? q.sentence : q.item.en} mic={mic} micBlocked={micBlocked} onListen={() => speakCheck(q.mode === "build" ? q.sentence : q.item.en)} />
                <div style={{ fontSize: 11.5, color: "#2f5f55", fontWeight: 700, margin: "8px 0" }}>（🎤 唸給蒙蒙聽是加分練習、不影響星星；點任何一個選項可以再聽一次）</div>
          ...
          <div>
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <div style={{ fontWeight: 900, fontSize: 17 }}>☕ 跟蒙蒙聊天</div>
              ...
              <div style={{ maxHeight: 320, overflowY: "auto", padding: "4px 2px" }}>
                {chat.map((m, i) => <Bubble key={i} who={m.who} en={m.en} zh={m.zh} onClick={() => speak(m.en)} tail={m.who === "owl" ? "🔊" : ""} />)}
                {chatBusy && <div style={{ fontSize: 12.5, color: K.sub, fontWeight: 700, marginTop: 8 }}>🦉 蒙蒙想一想…</div>}
              </div>
              <div style={{ fontSize: 11.5, color: K.sub, fontWeight: 900, margin: "10px 0 4px" }}>💡 你可以這樣說（點一下＝說出來）：</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {chatHints.map((h, i) => (
                  <button key={i} onClick={() => chatSend(h)} disabled={chatBusy} style={{ fontFamily: MONO, fontWeight: 900, fontSize: 13.5, cursor: "pointer", background: K.chick, border: `2.5px solid ${K.edge}`, borderRadius: 999, padding: "7px 12px", opacity: chatBusy ? 0.5 : 1 }}>{h}</button>
                ))}
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 12, flexWrap: "wrap" }}>
                {hasSTT() && !micBlocked && (
                  <Btn onClick={chatListen} disabled={mic.status === "listening" || chatBusy} color={mic.status === "listening" ? K.red : K.sun} edge={mic.status === "listening" ? "#b23238" : K.sunEdge} fg={mic.status === "listening" ? "#fff" : K.ink} style={{ fontSize: 15, padding: "12px 18px" }}>
                    {mic.status === "listening" ? "🎙️ 蒙蒙在聽…" : "🎤 我來說"}
                  </Btn>
                )}
                <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") chatSend(chatInput); }} placeholder="或打字（爸媽幫忙也可以）…"
                  style={{ flex: 1, minWidth: 150, fontFamily: FONT, fontSize: 14, padding: "10px 12px", borderRadius: 14, border: `2.5px solid ${K.edge}` }} />
                <Btn onClick={() => chatSend(chatInput)} disabled={chatBusy || !chatInput.trim()} color={K.teal} edge={K.tealEdge} fg="#fff" style={{ fontSize: 14, padding: "10px 14px" }}>送出</Btn>
              </div>
              {mic.error && <div style={{ fontSize: 12, color: K.red, fontWeight: 700, marginTop: 8 }}>{mic.error}</div>}
              <div style={{ fontSize: 11, color: K.sub, marginTop: 10 }}>蒙蒙由 Claude 即時回話（只用簡單英文、附中文），對話不會被儲存；語音辨識由瀏覽器提供，在 Claude 預覽視窗裡麥克風可能被擋住。</div>
            </Card>
          </div>
        )}
...
// ---------- 🎤 跟讀面板（教室第四步／挑戰揭曉共用）----------
function MicPanel({ target, mic, micBlocked, onListen, onDone, onSkip, compact }) {
  const t = mic.result ? mic.result.tier : null;
  const canMic = hasSTT() && !micBlocked;
  return (
    <div style={{ background: "#fff", border: `3px solid ${K.edge}`, borderRadius: 18, padding: compact ? "10px 12px" : "14px 14px", marginTop: 10 }}>
      {!compact && <div style={{ fontSize: 14, fontWeight: 900, color: K.sub, marginBottom: 6 }}>🎤 開口練習：對著麥克風唸這句給蒙蒙聽</div>}
      <div style={{ fontFamily: MONO, fontWeight: 900, fontSize: compact ? 15 : 19 }}>{target}</div>
      <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginTop: 10 }}>
        {canMic ? (
          <Btn onClick={onListen} disabled={mic.status === "listening"} color={mic.status === "listening" ? K.red : K.sun} edge={mic.status === "listening" ? "#b23238" : K.sunEdge} fg={mic.status === "listening" ? "#fff" : K.ink} style={{ fontSize: 15, padding: "12px 20px" }}>
            {mic.status === "listening" ? "🎙️ 蒙蒙在聽…（說吧）" : mic.result ? "🎤 再唸一次" : "🎤 我來唸"}
          </Btn>
        ) : (
          <div style={{ fontSize: 12, color: K.sub, fontWeight: 700, maxWidth: 280 }}>{micBlocked ? "這裡的麥克風被擋住了（放到自己的網頁就可以用）——先跟著蒙蒙唸幾次也很棒！" : "這個瀏覽器沒有語音辨識——先跟著蒙蒙唸幾次也很棒！"}</div>
        )}
        {onDone && <Btn onClick={onDone} color={t && t !== "retry" ? K.teal : "#fff"} edge={t && t !== "retry" ? K.tealEdge : K.edge} fg={t && t !== "retry" ? "#fff" : K.ink} style={{ fontSize: 14, padding: "12px 16px" }}>
          {t === "great" ? "🌟 完成！" : t === "close" ? "👍 完成" : (canMic ? "跳過也可以 ▶" : "我唸過了 ▶")}
        </Btn>}
        {onSkip && <Btn onClick={onSkip} color="#fff" edge={K.edge} style={{ fontSize: 13, padding: "10px 12px" }}>算了，下一題</Btn>}
      </div>
      {mic.result && (
        <div style={{ marginTop: 10, animation: "popIn .35s ease" }}>
          <div style={{ fontSize: 12, color: K.sub }}>蒙蒙聽到：<span style={{ fontFamily: MONO, fontWeight: 700, color: K.ink }}>{mic.result.text || "（沒聽清楚）"}</span></div>
          <div style={{ fontSize: 15, fontWeight: 900, marginTop: 2, color: t === "great" ? K.greenD : t === "close" ? K.sunEdge : K.sub }}>
            {t === "great" ? "🌟 太棒了！蒙蒙聽得清清楚楚！" : t === "close" ? "👍 很接近！再唸一次會更棒" : "🐢 蒙蒙沒聽清楚，慢慢再說一次？"}
          </div>
        </div>
      )}
      {mic.error && <div style={{ fontSize: 12, color: K.red, fontWeight: 700, marginTop: 8 }}>{mic.error}</div>}
          ...
          {!compact && (
            <>
              ...
              <div style={{ fontSize: 11.5, color: K.sub }}>
                <b>老實說明</b>：36 組一輪對話（八大主題、一年級核心溝通功能、標準美式用法）。教室每組三步——聽對話、逐字學（每個字有簡短中文）、照樣造句（換一個字＝一句新話，17 組有樣板）；挑戰只考教過的，五種題型隨熟練度升級。記憶模型是遺忘曲線＋測驗效應＋間隔＋興趣電量的<b>近似</b>，每答一題校正一次；2.0 加入即時語音互動：教室第四步「開口練習」與挑戰揭曉的「唸給蒙蒙聽」用瀏覽器語音辨識做<b>寬鬆比對</b>（相似度 ≥50% 就給「接近」的鼓勵；≥80% 且字詞命中 ≥75% 才算「很棒」），只鼓勵、不扣星星——因為兒童語音的辨識率本來就偏低，這不是發音評分；「跟蒙蒙聊天」由 Claude 即時生成一句簡單英文＋中文＋建議句，對話不儲存。麥克風在 Claude 預覽視窗裡可能被沙箱擋住，放到自己的網頁即可；沒有麥克風時所有語音功能都可跳過或改用點選／打字。換卡與「下一題」都會等語音真的說完。已上課 {seeds} 堂、挑戰 {sessions} 場；進度自動儲存。
              </div>
